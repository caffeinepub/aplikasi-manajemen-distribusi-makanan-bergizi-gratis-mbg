...
export default function LaporanPage() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      let data: Awaited<ReturnType<typeof actor.getDataUntukLaporan>>;
      if (filterKategori === "semua") {
        data = await actor.getDataUntukLaporan(startTime, endTime);
      } else {
        data = await actor.getLaporanByKategori(
          filterKategori as Kategori,
          startTime,
          endTime,
        );
      }
      ...
      if (!data.distribusi || data.distribusi.length === 0) {
        toast.error(
          "Tidak ada data distribusi dalam rentang tanggal yang dipilih.",
        );
      ...
      doc.text(
        "LAPORAN DISTRIBUSI MAKANAN BERGIZI GRATIS",
        pageWidth / 2,
        yPos,
        { align: "center" },
      );
      ...
      yPos += 5;
      doc.text(
        `Total Sasaran Terlibat: ${new Set(data.distribusi.map((d) => d.idSasaran.toString())).size}`,
        margin,
        yPos,
      );
      yPos += 5;
      doc.text(
        `Total Paket Terdistribusi: ${data.distribusi.reduce((sum, d) => sum + Number(d.jumlahPaket), 0)}`,
        margin,
        yPos,
      );
      yPos += 12;
      ...
      data.distribusi.forEach((dist, index) => {
        ...
        doc.text(
          truncateText(
            getKategoriLabel(
              (sasaran?.kategori ?? "tidakDitentukan") as Kategori,
            ),
            15,
          ),
          xPos,
        ...
        xPos += colWidths.tanggal;
        doc.text(
          truncateText(getStatusLabel(dist.statusDistribusi), 15),
          xPos,
          yPos,
        );
        xPos += colWidths.status;
      ...
      const kategoriSuffix =
        filterKategori !== "semua" ? `_${filterKategori}` : "";
      const filename = `laporan_MBG_${formatDateForFilename(startDate)}_${formatDateForFilename(endDate)}${kategoriSuffix}.pdf`;

      doc.save(filename);

      toast.success(
        `Laporan PDF berhasil diunduh! (${data.distribusi.length} data)`,
      );
    } catch (error) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Generate laporan PDF distribusi MBG
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Pilih periode tanggal dan kategori untuk menghasilkan laporan
            distribusi
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <Alert>
            ...
            <AlertDescription>
              Laporan akan mencakup semua data distribusi dalam periode yang
              dipilih. Pastikan tanggal mulai lebih awal dari tanggal akhir.
            </AlertDescription>
          </Alert>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-lg bg-emerald-50 p-4">
            <p className="text-sm text-emerald-800">
              <strong>Catatan:</strong> Laporan PDF akan mencakup tabel
              terstruktur dengan kolom: No, Nama Sasaran, Kategori, Paket,
              Jumlah, Tanggal, Status, dan Keterangan. File akan otomatis diberi
              nama berdasarkan rentang tanggal dan kategori yang dipilih.
            </p>
          </div>
        </CardContent>
      </Card>
