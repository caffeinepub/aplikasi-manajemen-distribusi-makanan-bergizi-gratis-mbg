  ...
  const toggleDesaAll = (group: VillageGroup) => {
    ...
    if (allSelected) {
      ...
      const toAdd = groupIds.filter(
        (id) => !selectedIds.some((sid) => sid === id),
      );
    ...
    <div className="space-y-3">
        ...
        <div className="p-2">
          {villageGroups.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">
                {searchTerm
                  ? "Tidak ada sasaran yang sesuai"
                  : "Belum ada data sasaran aktif"}
              </p>
            </div>
          ) : (
                    ...
                    <CollapsibleTrigger asChild>
                      ...
                      >
                        ...
                        <Badge variant="outline" className="text-xs">
                          {
                            group.sasaran.filter((s) =>
                              selectedIds.some((sid) => sid === s.id),
                            ).length
                          }
                  ...
                  <CollapsibleContent>
                    <div className="ml-6 space-y-0.5 border-l border-emerald-100 pl-3">
                      {group.sasaran.map((sasaran) => {
                        const checkboxId = `sasaran-${sasaran.id.toString()}`;
      ...
      {activeSasaran.length > 0 && (
        <p className="text-xs text-muted-foreground">
          {activeSasaran.length} sasaran aktif tersedia • {selectedIds.length}{" "}
          dipilih
        </p>
      )}
    </div>
